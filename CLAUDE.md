...
- Node.js: 20.x
- pnpm: 9.x
- Figma: デザインツール（MCP サーバー経由で接続済み。Figma Make でデザイン生成）
...
- デザインは事前に存在しない。UI 実装時は以下のワークフローで Figma Make を活用する:
  1. UI 要件からデザインプロンプト（画面構成・コンポーネント・配色・レイアウト等）を作成し、ユーザーに提示する
  2. ユーザーが Figma Make（figma.com/make/）でデザインを生成
  3. ユーザーから Figma Make URL を受け取ったら、`get_design_context` でデザイン情報を取得
  4. 返却される参照コードはあくまで参考。プロジェクトの既存パターンに合わせて適応してコンポーネントを生成する
- Code Connect マッピングが存在する場合は、マッピング先の既存コンポーネントを優先的に使用する
- デザイントークン（CSS 変数）はプロジェクトの Tailwind 設定にマッピングすること
...
- IMPORTANT: Figma デザインからコード生成時は、`components/ui/` の既存コンポーネントと Tailwind トークンを優先して再利用すること。生の hex 値や絶対座標をそのまま使わない
- .env ファイルは絶対にコミットしない
...
- Figma デザインからコンポーネント生成 → `design-to-code`
