...
- **使うとき:** 「テストを書いて」「カバレッジを上げて」「テストが落ちてるので直して」
- **使わないとき:** 機能実装やコードレビュー
...
セキュリティ、パフォーマンス、アクセシビリティの監査を一括で実施します。

- **モデル:** sonnet
- **監査領域:**
  - セキュリティ: OWASP Top 10、認証・認可、入力バリデーション、機密情報管理
  - パフォーマンス: Core Web Vitals（LCP/INP/CLS）、バンドルサイズ、レンダリング最適化
  - アクセシビリティ: WCAG 2.1 AA、セマンティック HTML、ARIA、キーボード操作
- **使うとき:** 「セキュリティチェックして」「パフォーマンスを改善したい」「アクセシビリティ監査して」「リリース前のチェックをして」
- **使わないとき:** 機能実装、コードレビュー、テスト作成
...
Figma デザインから React コンポーネントを生成します。

- **モデル:** sonnet
- **得意領域:** Figma MCP 連携、デザイントークンの変換、shadcn/ui コンポーネントへのマッピング
- **使うとき:** Figma URL からコンポーネントを生成したいとき、デザインプロンプトを作成したいとき
- **使わないとき:** デザインなしのコンポーネント作成（`/generate-component` を使用）

```
この Figma デザインからコンポーネントを生成して
```
...
- **使うとき:** 「レビューして」「PRを確認して」「マージ前の最終確認をして」
- **使わないとき:** 機能実装、テスト作成、監査（セキュリティ/パフォーマンス）
